...
- **`services.analysis_service.AnalysisService`:** Encapsulates the core business logic and prompt engineering. It receives User Story IDs or raw text, constructs prompt with strict JSON schema enforcement, and delegates the API call. 
...
- **Automated Testing:** The codebase is evaluated using a deterministic testing suite built upon the `pytest` and `pytest-mock` frameworks, currently achieving a 100% passing rate across dedicated unit tests. The testing architecture is contained within the `tests/` directory, organized to mirror the application's modular logic (categorized into `cli/`, `clients/`, `helpers/`, and `services/`). To ensure strict environmental independence and eliminate reliance on external infrastructure—such as running Docker containers or an active Ollama inference engine—all network communications and file system I/O operations are comprehensively mocked. 
...
### 5.5 Long processing time
Long processing can be experienced or eventually timeout while running in docker environment. This might be caused by default resource limits for docker, especially RAM usage. Increasing RAM to 24 GB in the test case significantly improved performance and reduced time required to produce results.
